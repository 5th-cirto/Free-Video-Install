...
## 13. 前端工作台 V2（紧凑版，2026-03）

### 13.1 设计目标

- 在不改变后端 API 协议的前提下，完成前端工作台大改版。
- 桌面端在首屏实现 `40%:60%` 双栏布局（左侧下载与视频信息，右侧 AI 总结主视图）。
- 移动端提升信息密度与阅读优先级，确保“小屏先看到总结核心内容”。
- 保留并稳定现有能力：解析、下载、任务中心、SSE 总结、导图预览与 SVG 导出、字幕下载。

### 13.2 信息架构与布局策略

```mermaid
flowchart LR
  inputArea[LinkInputAndParse] --> leftPane[VideoMetaAndDownload]
  inputArea --> rightPane[AISummaryMain]
  rightPane --> tabs[SummaryOutlineMindmapSubtitle]
  tasks[TaskCenterSecondary] --> workspace[SingleScreenWorkspace]
```

- 桌面端：
  - 主工作区采用 `2fr 3fr` 网格（约等于 `40%:60%`）。
  - 左栏聚焦输入、解析结果、格式选择与下载操作。
  - 右栏聚焦 AI 总结页签内容，支持分区内滚动。
- 移动端：
  - 改为单列布局，默认将下载区域置于总结区域之前，缩短“输入 -> 解析 -> 下载/总结”操作路径。
  - 降低间距与字号层级，控制单屏可见信息量。

### 13.3 交互策略

- 保持“解析后自动触发 AI 总结”的一体化流程。
- 任务中心从首屏主区降级为次级区：默认折叠，展示任务统计摘要，按需展开详情。
- 长内容（总结正文、字幕分段、任务列表）进入容器内滚动，避免整页被拉长。

### 13.4 UI 视觉基调

- 视觉风格切换为浅色简洁商务风，减少强装饰背景和高对比干扰。
- 强化信息层级：标题、关键状态、主要操作、结果内容分层清晰。
- 保留必要品牌识别，不再以深色霓虹风作为主导。

### 13.5 验收口径（V2）

- 桌面端首屏可同时看到左栏视频信息卡与右栏总结主区域。
- 主流程不回退：解析 -> 下载 -> AI 流式总结 -> 页签切换均可正常使用。
- 任务中心可折叠并可完整查看任务详情，不影响“打开文件位置”等操作。
- 移动端在常见窄屏下具备可读性与可操作性，不出现关键按钮遮挡/溢出。
